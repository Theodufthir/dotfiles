...
const Device = (device: Bluetooth.Device) =>
  <box spacing={10}>
    <box class="indicator">
      <TablerIcon icon={createBinding(device, "icon").as(i => i?.split("-").pop() ?? "bluetooth")}/>
    </box>
    <Button
      class="highlightable"
      hexpand
      ...
      <label label={
        createBinding(device, "alias").as(alias => alias ?? device.address)
      }/>
    </Button>
  </box>
...
const BluetoothPopup = (props: PopupWindowProps) =>
  <PopupWindow
    ...
    <FilteredSections
      class="base"
      source={createBinding(bluetooth, "devices")}
      template={Device}
      ...
      sections={[
        ...
        ...props,
        hideIfEmpty: true,
        props: { orientation: Gtk.Orientation.VERTICAL, class: "device-list" }
      }))}
    />
  </PopupWindow>
